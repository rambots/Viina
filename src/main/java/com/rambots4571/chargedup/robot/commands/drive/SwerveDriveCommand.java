...
public class SwerveDriveCommand extends CommandBase {
  ...
  public void execute() {
    double translationVal =
        MathUtil.applyDeadband(translationSupplier.getAsDouble(), Settings.STICK_DEADBAND);
    double strafeVal =
        MathUtil.applyDeadband(strafeSupplier.getAsDouble(), Settings.STICK_DEADBAND);
    double rotationVal =
        MathUtil.applyDeadband(rotationSupplier.getAsDouble(), Settings.STICK_DEADBAND);

    driveTrain.drive(
        new Translation2d(translationVal, strafeVal).times(DriveConstants.kMaxSpeedMetersPerSecond),
        (rotationVal * DriveConstants.kMaxAngularSpeedMetersPerSecond),
